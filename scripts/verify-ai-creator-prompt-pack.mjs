...
assert.match(page, /Checkout is live on Gumroad/);
...
assert.ok(customPromptMailLink);
const decodedCustomPromptEmail = decodeURIComponent(customPromptMailLink[1]);
assert.match(decodedCustomPromptEmail, /Niche\/brand:/);
assert.match(decodedCustomPromptEmail, /Prompt quantity:/);
assert.match(decodedCustomPromptEmail, /Deadline:/);
assert.match(decodedCustomPromptEmail, /Goal for the prompts:/);
